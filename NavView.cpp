...
void CNavView::OnInitialUpdate() 
{
	...
	cus.DeflateRect(5,5);
	//Deflect(10, 10, 10, 10);
    CRect box(0, rgap, cgap,2* rgap);
	box.InflateRect(-5,-5);
	CRect item(0,2 * rgap,cgap,3 * rgap);
	item.InflateRect(-5,-5);
	CRect sale(0,3* rgap,cgap,rgap*4);
	sale.InflateRect(-5,-5);
	CRect user(0,4*rgap,cgap,rgap*5);
	user.InflateRect(-5,-5);
	CRect money(0,5*rgap,cgap,rgap*6);
	...
	this->SetWindowPos(&CWnd::wndTop, 0, 0, 100, 600, SWP_NOMOVE|SWP_DRAWFRAME|SWP_NOREDRAW);
